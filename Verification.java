		...
		if (sh.sourceContent.size() != sh.destinationContent.size()) {
			...
			for (Map.Entry<String, Hashstructure> htEntries : sh.sourceContent.entrySet()) {
				if (sh.destinationContent.containsKey(htEntries.getKey())) {
					...
					if ((sh.destinationContent.get(htEntries.getKey()).readdata() == null && htEntries.getValue().readdata() != null) ||
							(sh.destinationContent.get(htEntries.getKey()).readdata() != null && htEntries.getValue().readdata() == null)) {
						BADflag = 1;
						System.out.println("Data mismatch at Key: " + htEntries.getKey());
						continue;
					}
					
					if (sh.destinationContent.get(htEntries.getKey()).readmetadata() == null || htEntries.getValue().readmetadata() == null) {
						BADflag = 1;
						System.out.println("Metadata found to be NULL for Key: " + htEntries.getKey());
						continue;
					}
